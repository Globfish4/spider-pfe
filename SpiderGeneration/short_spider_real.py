...
class DynamSpider():
    ...
    def create_prims(self):
        ...
        paws_phys_material = PhysicsMaterial(
            prim_path="/World/Physics_Materials/paw_material", 
            dynamic_friction=0.1, 
            static_friction=0.1, 
            restitution=0.0, 
            )

        self._body12.apply_physics_material(physics_material=paws_phys_material)
        self._body22.apply_physics_material(physics_material=paws_phys_material)
        self._body32.apply_physics_material(physics_material=paws_phys_material)
        self._body42.apply_physics_material(physics_material=paws_phys_material)
    ...
    def add_servos(self):
        ...
        self.servo12 = DynamXLSim(self.world, np.array([0.255, 0, 0.018]), np.array([np.pi/2, 0, -np.pi/2]), name="Spider/Servo12", id="12")
        self.servo22 = DynamXLSim(self.world, np.array([0, 0.255, 0.018]), np.array([np.pi/2, 0, 0]), name="Spider/Servo22", id="22")
        self.servo32 = DynamXLSim(self.world, np.array([-0.255, 0, 0.018]), np.array([np.pi/2, 0, np.pi/2]), name="Spider/Servo32", id="32")
        self.servo42 = DynamXLSim(self.world, np.array([0, -0.255, 0.018]), np.array([np.pi/2, 0, -np.pi]), name="Spider/Servo42", id="42")
    ...
    def attach_servos(self):
        self.servo11.attach_body_to_rotor(prim_path1="/Spider/Abdomen", prim_path2="/Spider/Paw11", 
                                          ...
                                          localpos0B=Gf.Vec3f(0.0, 0.0195, -0.043), localrot0B=np.array([-90, 0, 180]))
        self.servo21.attach_body_to_rotor(prim_path1="/Spider/Abdomen", prim_path2="/Spider/Paw21", 
                                          ...
                                          localpos0B=Gf.Vec3f(-0.0195, 0.0, 0.043), localrot0B=np.array([90, 0, 90]))
        self.servo31.attach_body_to_rotor(prim_path1="/Spider/Abdomen", prim_path2="/Spider/Paw31", 
                                          ...
                                          localpos0B=Gf.Vec3f(0.0, -0.0195, -0.043), localrot0B=np.array([-90, 0, 0]))
        ...
        self.servo12.attach_body_to_rotor(prim_path1="/Spider/Paw11", prim_path2="/Spider/Paw12", 
                                          localpos0A=Gf.Vec3f(0.0, -0.02, 0.038), localrot0A=np.array([0, -90, 90]),
        ...
        self.servo22.attach_body_to_rotor(prim_path1="/Spider/Paw21", prim_path2="/Spider/Paw22", 
                                          localpos0A=Gf.Vec3f(0.02, 0.0, -0.038), localrot0A=np.array([0, 90, 0]),
        ...
        self.servo32.attach_body_to_rotor(prim_path1="/Spider/Paw31", prim_path2="/Spider/Paw32", 
                                          localpos0A=Gf.Vec3f(0.0, 0.02, 0.038), localrot0A=np.array([-90, -90, 0]),
        ...
        self.servo42.attach_body_to_rotor(prim_path1="/Spider/Paw41", prim_path2="/Spider/Paw42", 
                                          localpos0A=Gf.Vec3f(-0.02, 0.0, -0.038), localrot0A=np.array([180, 90, 0]),
    ...
    def act(self, Nsteps, action):
        ...
        for _ in range(Nsteps):
            ...
            # print("## Joint efforts : ", joo)
            # print("mean : ", np.mean(joo))
    ...
    def walk(self, distance):
        ...
        while dist < distance:
            ...
            print("speed : ", dist/world.current_time)
...
def neutral_simulation(n, art):
    print("########### Starting Neutral Simulation #####################")
    for _ in range(n):
        # print("################################################## Step N°" + str(_))
        world.step(render=True)
        # print("## Joint efforts : ", art.get_measured_joint_efforts())
...
if __name__ == "__main__":
    ...
    # floor_physics_material = PhysicsMaterial(
    #     prim_path="/World/Physics_Materials/floor_material", dynamic_friction=0.2, static_friction=0.2, restitution=0.0
    # )
    # # arm.apply_physics_material(physics_material=physics_material_2)

    
    # floor = DynamicCuboid(
    #     prim_path="/World/Rob/floor", 
    #     position=np.array([0.0, 0.0, 0.01]),
    #     scale=np.array([20.0, 20.0, 0.02]),
    #     physics_material=floor_physics_material,
    # )
    

    spider = DynamSpider(world)
    print(spider.art.dof_names)

    # neutral_simulation(200, spider.art)
    # spider.stand_up()
    # neutral_simulation(10, spider.art)
    # spider.lay_down()
    neutral_simulation(10, spider.art)
    ...
    neutral_simulation(99999999, spider.art)
