...
class DynamSpider():
    ...
    def create_prims(self):
        ...
        self._body11 = DynamicCylinder(prim_path="/Spider/Paw11",
                            position=np.array([0.125, 0.125, 0.01]), 
                            orientation=euler_angles_to_quat(np.array([0, np.pi/2, np.pi/4])),
                            radius=0.01, 
                            height=0.075, 
                            mass=0.01)
        self._body12 = DynamicCylinder(prim_path="/Spider/Paw12",
                            position=np.array([0.23, 0.23, 0.01]), 
                            orientation=euler_angles_to_quat(np.array([0, np.pi/2, np.pi/4])),
                            radius=0.01, 
                            height=0.075, 
                            mass=0.01)
    ...
    def add_servos(self):
        self.servo11 = Dynam2XLSim(self.world, np.array([0.09, 0.09, 0.025]), np.array([0, -np.pi/2, np.pi/4]), name="Spider/Servo11", id="11")
        self.servo21 = Dynam2XLSim(self.world, np.array([-0.09, 0.09, 0.025]), np.array([0, -np.pi/2, np.pi/4*3]), name="Spider/Servo21", id="21")
        self.servo31 = Dynam2XLSim(self.world, np.array([-0.09, -0.09, 0.025]), np.array([0, -np.pi/2, np.pi/4*5]), name="Spider/Servo31", id="31")
        self.servo41 = Dynam2XLSim(self.world, np.array([0.09, -0.09, 0.025]), np.array([0, -np.pi/2, np.pi/4*7]), name="Spider/Servo41", id="41")
        
        self.servo12 = DynamXLSim(self.world, np.array([0.2, 0.2, 0.02]), np.array([np.pi/2, 0, -np.pi/4]), name="Spider/Servo12", id="12")
        self.servo22 = DynamXLSim(self.world, np.array([-0.2, 0.2, 0.02]), np.array([np.pi/2, 0, np.pi/4]), name="Spider/Servo22", id="22")
        self.servo32 = DynamXLSim(self.world, np.array([-0.2, -0.2, 0.02]), np.array([np.pi/2, 0, 3*np.pi/4]), name="Spider/Servo32", id="32")
        self.servo42 = DynamXLSim(self.world, np.array([0.2, -0.2, 0.02]), np.array([np.pi/2, 0, 5*np.pi/4]), name="Spider/Servo42", id="42")
    ...
    def attach_servos(self):
        self.servo11.attach_body_to_rotor(prim_path1="/Spider/Abdomen", prim_path2="/Spider/Paw11", 
                                          localpos0A=Gf.Vec3f(0.63837, 0.63837, 0.39), localrot0A=np.array([0, 0, 45]),
                                          localpos0B=Gf.Vec3f(-0.015, 0.0195, -0.0625), localrot0B=np.array([-90, 0, 180]))
        self.servo21.attach_body_to_rotor(prim_path1="/Spider/Abdomen", prim_path2="/Spider/Paw21", 
                                          localpos0A=Gf.Vec3f(-0.63837, 0.63837, 0.39), localrot0A=np.array([0, 0, 135]),
                                          localpos0B=Gf.Vec3f(-0.015, -0.0195, 0.0625), localrot0B=np.array([90, 0, 180]))
        self.servo31.attach_body_to_rotor(prim_path1="/Spider/Abdomen", prim_path2="/Spider/Paw31", 
                                          localpos0A=Gf.Vec3f(-0.63837, -0.63837, 0.39), localrot0A=np.array([0, 0, 225]),
                                          localpos0B=Gf.Vec3f(-0.015, -0.0195, 0.0625), localrot0B=np.array([90, 0, 180]))
        self.servo41.attach_body_to_rotor(prim_path1="/Spider/Abdomen", prim_path2="/Spider/Paw41", 
                                          localpos0A=Gf.Vec3f(0.63837, -0.63837, 0.39), localrot0A=np.array([0, 0, 315]),
                                          localpos0B=Gf.Vec3f(-0.015, 0.0195, -0.0625), localrot0B=np.array([-90, 0, 180]))


        self.servo12.attach_body_to_rotor(prim_path1="/Spider/Paw11", prim_path2="/Spider/Paw12", 
                                          localpos0A=Gf.Vec3f(-0.01, -0.02, 0.06407), localrot0A=np.array([90, -90, 0]),
                                          localpos0B=Gf.Vec3f(-0.01, 0.0, -0.07243), localrot0B=np.array([-180, 0, -90]))
        self.servo22.attach_body_to_rotor(prim_path1="/Spider/Paw21", prim_path2="/Spider/Paw22", 
                                          localpos0A=Gf.Vec3f(-0.01, 0.02, -0.06407), localrot0A=np.array([-90, 90, 0]),
                                          localpos0B=Gf.Vec3f(-0.01, 0.0, 0.07243), localrot0B=np.array([0, 0, 90]))
